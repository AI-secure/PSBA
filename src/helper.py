...
if __name__ == '__main__':
    ...
    cmd_base = 'CUDA_VISIBLE_DEVICES=%d python3 main_pytorch_multi.py --suffix casestudy --use_gpu --N_img 1 --mode test --plot_adv' %(args.gpu_id)
    task_args = [
        '--TASK imagenet',
        '--TASK celeba',
    ...
    pgens = [[], []]
    pgens[0] = ['naive', 'resize9408', 'DCT9408', 'PCA9408', 'AE9408', 'VAE9408', 'GAN9408']
    # pgens[1] = ['naive', 'resize9408', 'DCT9408', 'PCA9408', 'AE9408', 'VAE9408', 'DCGAN']
    pgens[1] = ['PCA9408',]
    # pgens[1] = ['naive']
    for _a in range(len(task_args)):
        if _a < 3:
            continue
        task_arg = task_args[_a]
        pgen_list = pgens[int(_a/2)]
        for pgen in pgen_list:
            print(time.time())
            cmd = "%s %s --pgen %s" %(cmd_base, task_arg, pgen)
            print(cmd)
            os.system(cmd)
