...
def preprocess_raw_imagenet():
    _size = (224, 224)
    rootdir = '/data/datasets/imagenet/ILSVRC2012/train'
    _cap = 300000
...
def preprocess_rnd_data(TASK, REFs, N_train, N_test):
    ...
    for i in tqdm(range(N_train_tot)):
        ...
        for j in range(N_rand):
            ...
            ori_path = '../raw_data/%s_%s/train_batch_%d.npy' % (TASK, REF, i)
            lnk_path = '../raw_data/%s_rnd/train_batch_%d.npy' %(TASK, cnt_train)
            if TASK == 'imagenet':
    ...
    for i in tqdm(range(N_test_tot)):
        ...
        for j in range(N_rand):
            ...
            ori_path = '../raw_data/%s_%s/test_batch_%d.npy' % (TASK, REF, i)
            lnk_path = '../raw_data/%s_rnd/test_batch_%d.npy' % (TASK, cnt_test)
            if TASK == 'imagenet':
                cmd = 'mv %s %s' %(ori_path, lnk_path) # only store the rnd set for imagenet
            else:
                cmd = 'ln -s %s %s' % (ori_path, lnk_path)
    ...
    #     REF = REFs[rand]
    #     ori_path = '../raw_data/%s_%s/train_batch_%d.npy' % (TASK, REF, i)
        # cur_data = np.load('../raw_data/%s_%s/train_batch_%d.npy' % (TASK, REF, i))
        # np.save(path + '/train_batch_%d.npy' % i, cur_data)
    # for i in tqdm(range(math.ceil(N_test / BATCH_SIZE))):
    #     rand = np.random.randint(len(REFs))
    #     REF = REFs[rand]
    #     ori_path = '../raw_data/%s_%s/test_batch_%d.npy' % (TASK, REF, i)
        # cur_data = np.load('../raw_data/%s_%s/test_batch_%d.npy' % (TASK, REF, i))
        # np.save(path + '/test_batch_%d.npy' % i, cur_data)


def validate_data(TASK, REF, mode, idx):
    data_path = '../raw_data/%s_%s/%s_batch_%d.npy'
    X = np.load(data_path%(TASK, REF, mode, idx))
    X = utils.validate(X)
    # new_data_path = '../raw_data/%s_%s/reg_%s_batch_%d.npy'
    np.save(data_path%(TASK, REF, mode, idx), X)
