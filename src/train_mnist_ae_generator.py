...
def epoch_train(REF, model, optimizer, BATCH_SIZE=32, mounted=False):
    if mounted:
        data_dir = '../raw_data/%s_%s' %(TASK, REF)
        if img_size != 28:
            data_dir = '../raw_data/%s_%d_%s' %(TASK, img_size, REF)
    else:
        data_dir = '../raw_data/%s_%s' %(TASK, REF)
        if img_size != 28:
            data_dir = '../raw_data/%s_%d_%s' % (TASK, img_size, REF)
...
if __name__ == '__main__':
    ...
    if not args.mounted:
        if img_size == 28:
            utils.plot_training_data(data_path='../raw_data/%s_%s/train_batch_%d.npy' %(TASK, 'rnd', 0),
                                     img_name='%s_%d_%s_%d' %(TASK, 28, 'rnd', 0), x_shape=(28, 28))
        else:
            utils.plot_training_data(data_path='../raw_data/%s_%d_%s/train_batch_%d.npy' %(TASK, img_size, 'rnd', 0),
                                     img_name='%s_%d_%s_%d' %(TASK, img_size, 'rnd', 0), x_shape=(224, 224))
