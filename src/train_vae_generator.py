...
def epoch_train(REF, model, optimizer, BATCH_SIZE = 32, mounted=False):
    ...
    if mounted:
        ...
        data_path = '../raw_data/%s_%s/train_batch_%d.npy'
    # N_train = 8750
    # data_path = '../raw_data/imagenet_avg/train_batch_%d.npy'
    #test: 625; train: 8750
...
def epoch_eval(REF, model, BATCH_SIZE=32, mounted=False):
    # N_test = 625
    # data_path = '../raw_data/imagenet_avg/test_batch_%d.npy'
    if TASK == 'celeba':
        N_test = 624
        if REF == 'rnd':
            N_test = 625
...
if __name__ == '__main__':
    ...
    if not args.mounted:
        if n_channels == 3:
            utils.plot_training_data(data_path='../raw_data/%s_%s/train_batch_%d.npy'%(TASK, 'res50', 0), img_name='%s_%s_%d'%(TASK, 'res50', 0), x_shape=(n_channels, 224, 224))
        else:
            utils.plot_training_data(data_path='../raw_data/%s_%s/train_batch_%d.npy' % (TASK, 'res50', 0),
                                     img_name='%s_%s_%d' % (TASK, 'res50', 0), x_shape=(224, 224))
        # utils.plot_training_data(data_path='../raw_data/%s_%s/train_batch_%d.npy', TASK=TASK, REF='res50', idx=0)
