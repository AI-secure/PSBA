...
from models import PGenerator
from models import Cifar10AEGenerator
from models import EAGenerator
from models import SignOptGenerator
...
def load_pgen(task, pgen_type, args):
    ...
    #         p_gen = DCTGenerator(factor=8.0, preprocess=((2,0,1),0,1))
    #     elif pgen_type == 'DCT4107':
    #         p_gen = DCTGenerator(factor=6.0, preprocess=((2,0,1),0,1))
    #     elif pgen_type == 'DCT9408':
    #         p_gen = DCTGenerator(factor=4.0, preprocess=((2,0,1),0,1))
    #     elif pgen_type == 'DCT16428':
    #         p_gen = DCTGenerator(factor=3.0, preprocess=((2,0,1),0,1))
    #
    ...
    if task == 'imagenet' or task == 'celeba' or task == 'celebaid' or task == 'celeba2':
        ...
        elif pgen_type == 'evolution':
            p_gen = EAGenerator(dimension_reduction = (args.reduction,args.reduction), x_shape=(224,224,3))
        elif pgen_type == 'sign_opt':
            p_gen = SignOptGenerator(alpha = 0.2, beta = 0.001)
        elif pgen_type == 'DCT2352':
        ...
        elif pgen_type == 'PGAN9408':
            p_gen = PGenerator(dimLatent=9408, n_channels=3,device=args.device,dct_factor = args.dct_factor,topk = args.topk)
            full_scale = [256,256,128,64,32,16]
            p_gen.loadScale(full_scale[:args.scale])
            p_gen.setNewAlpha(1)
            p_gen.eval()   
            if args.topk:
                p_gen.load('./gen_models/pca_pgan_gen_%s_%d.npy' % (task, 9408))
            else:
                p_gen.load_state_dict(torch.load('./gen_models/%s_gradient_pgan_%d_generator_u%d.model' % (task, 9408,args.scale),map_location = args.device))
        ...
        if args.cifar10_img_size==32:
            ...
            elif pgen_type.startswith('PGAN'):
                N_Z = int(pgen_type[4:])
                p_gen = PGenerator(dimLatent=N_Z, n_channels=n_channels,device=args.device,dct_factor = args.dct_factor,topk = args.topk)
                full_scale = [256,256,128,64,32,16]
                p_gen.loadScale(full_scale[:args.scale])
                p_gen.setNewAlpha(1)
                p_gen.eval()
                if args.topk:
                    p_gen.load('./gen_models/pca_pgan_gen_%s_%d.npy' % ('cifar10_224', 9408))
                else:
                    p_gen.load_state_dict(torch.load('./gen_models/%s_gradient_pgan_%d_generator_u%d.model' % (model_file_name, 9408,args.scale),map_location=args.device))
        ...
        if args.mnist_img_size == 28:
            if pgen_type == 'naive':
                ...
                p_gen = EAGenerator(dimension_reduction = (args.reduction,args.reduction), x_shape=(224,224,1))
                ...
                assert 0
                # p_gen.load_state_dict(torch.load('./gen_models/%s_%d_gradient_gan_%d_generator.model' % (task, args.mnist_img_size, int(pgen_type[3:]))))

            elif pgen_type.startswith('VAE'):
                print("Not implemented yet")
                assert 0
                # p_gen.load_state_dict(torch.load('./gen_models/%s_%d_gradient_vae_%d_generator.model'
                #                                  %(task, args.mnist_img_size, int(pgen_type[3:]))))
            ...
            if pgen_type == 'naive':
                ...
                p_gen = PCAGenerator(N_b=N_b, approx=approx, basis_only=True)
                # p_gen.load('./gen_models/pca_gen_%s_%d.npy' % (model_file_name, 9408))
                p_gen.load('./gen_models/pca_gen_%s_%d.npy' % ('mnist_224', 9408))
                ...
                p_gen = PGenerator(dimLatent=9408, n_channels=1,device=args.device, dct_factor = args.dct_factor, topk = args.topk)
                full_scale = [256,256,128,64,32,16]
                p_gen.loadScale(full_scale[:args.scale])
                p_gen.setNewAlpha(1)
                p_gen.eval()
                if args.topk:
                    p_gen.load('./gen_models/pca_pgan_gen_%s_%d.npy' % ('mnist_224', 9408))
                else:
                    p_gen.load_state_dict(torch.load('./gen_models/%s_gradient_pgan_%d_generator_u%d.model' % (model_file_name, 9408,args.scale),map_location=args.device))
...
def celeba_attack(args, N_img, mounted=False):
    ...
    if not mounted:
        list_attr_path = '../raw_data/celebA/list_attr_celeba.txt'
        img_data_path = '../raw_data/celebA/img_align_celeba'
    else:
        list_attr_path = '../raw_data/data/celebA/list_attr_celeba.txt'
        img_data_path = '../raw_data/data/celebA/img_align_celeba'
...
def mnist_attack(args, N_img, num_class, mounted=False):
    ...
    import torchvision
    dataset = torchvision.datasets.MNIST(root='../raw_data/', train=False, download=True, transform=transform)
    src_images, src_labels = [], []
    tgt_images, tgt_labels = [], []
    used_ids = set()
    while (len(src_images) < N_img):
...
def nclass_attack(args, N_img, task, num_class, mounted=False):
    ...
    if task == 'dogcat':
        ...
        dataset = DogCatDataset(root_dir='../raw_data/dogcat', mode='test', transform=transform)
    elif task == 'celeba':
...
# def celeba_attack_old(args, N_img):
