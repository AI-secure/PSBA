...
if __name__ == '__main__':
    ...
    parser.add_argument('--mnist_img_size', type=int, default=224) # resize mnist img
    ...
    parser.add_argument('--cifar10_img_size', type=int, default=224) # resize cifar10
    ...
    parser.add_argument('--topk', type=int, default=0, help = 'the topk main components on PGAN')
    parser.add_argument('--scale', type=int, default=6, help = 'the scale of PGAN') 
    ...
    for PGEN in [args.pgen, ]:
        ...
        if PGEN == 'evolution':
            ...
            maxN = 1
            initN = 1 
        elif TASK == 'imagenet' or TASK == 'celeba' or TASK == 'celebaid' or TASK == 'dogcat2' or TASK == 'celeba2':
            if PGEN == 'naive':
                ITER = 100
                maxN = 100
                initN = 100
        ...
        with tqdm(range(N_img)) as pbar:
            for i in pbar:
                ...
                # print(src_image)
                # if args.suffix == 'casestudy':
                #     file_name_suffix = '%s_%s_%s'%(TASK, PGEN, args.suffix)
                #     np.save('BAPP_result/src_img_%d_%s.npy' %(i, file_name_suffix), src_image)
                #     np.save('BAPP_result/tgt_img_%d_%s.npy' %(i, file_name_suffix), tgt_image)
...
#                 pbar.set_description("src label: %d; tgt label: %d; rho: %.4f"%(src_label, tgt_label, rho))
                ...
                for _ in range(N_repeat):
                    ...
                    adv = attack(tgt_image, tgt_label, starting_point = src_image, iterations=ITER,
                                 stepsize_search=args.stepsize_search, unpack=False, max_num_evals=maxN,
